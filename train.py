...
def _load_model(
    ...
    compression_model = load_compression_model(
        model_name, device=device, cache_dir=model_path)
...
def train(
    ...
    dataset_path = '/media/malcolm/AE423D9C423D6A6B/audiocraft/src/train_data'
    ...
    #     subprocess.run(["mkdir", "-p", '/src/train_data'], check=True)
    ...
    # from paper
    optimizer = AdamW(model.lm.parameters(), lr=learning_rate,
    ...
    for epoch in range(num_epochs):
        for batch_idx, (audio, label) in enumerate(train_dataloader):
            ...
            torch.nn.utils.clip_grad_norm_(
                model.lm.parameters(), 1.0)  # clip gradients
            ...
            if save_models:
                if current_step % save_step == 0:
                    torch.save(model.lm.state_dict(),
                               f"{output_dir}/{timestamp}_lm_{current_step}.pt")
